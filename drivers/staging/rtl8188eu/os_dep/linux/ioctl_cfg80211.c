...
struct cfg80211_bss *rtw_cfg80211_inform_bss(_adapter *padapter, struct wlan_network *pnetwork)
{
	...
	if (pbuf == NULL) {
		pr_info("%s pbuf allocate failed  !!\n", __func__);
		return bss;
	}
	...
	if (bssinf_len > buf_size) {
		pr_info("%s IE Length too long > %zu byte\n", __func__, buf_size);
		goto exit;
	}
	...
	*((u64 *)(pbuf + len)) = cpu_to_le64(notify_timestamp);
	...
	if (unlikely(!bss)) {
		pr_info(FUNC_ADPT_FMT" bss NULL\n", FUNC_ADPT_ARG(padapter));
		goto exit;
	}
		...
		if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) == _TRUE) {
			...
			if (cur_network) {
				if (!rtw_cfg80211_inform_bss(padapter, cur_network))
					pr_info(FUNC_ADPT_FMT" inform fail !!\n", FUNC_ADPT_ARG(padapter));
				else
					pr_info(FUNC_ADPT_FMT" inform success !!\n", FUNC_ADPT_ARG(padapter));
			} else {
				pr_info("cur_network is not exist!!!\n");
				return ;
			}
		} else {
			if (scanned == NULL) {
			...
			if (_rtw_memcmp(&(scanned->network.Ssid), &(pnetwork->Ssid), sizeof(NDIS_802_11_SSID)) == _TRUE
				...
				pr_info("scanned & pnetwork compare fail\n");
				rtw_warn_on(1);
			}
		}

		if (!rtw_cfg80211_check_bss(padapter))
			pr_info(FUNC_ADPT_FMT" BSS not found !!\n", FUNC_ADPT_ARG(padapter));
	}
	...
	, const u8 *mac_addr, struct key_params *params)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE) {
		...
		pr_info("error! fw_state=0x%x, iftype=%d\n", pmlmepriv->fw_state, rtw_wdev->iftype);
