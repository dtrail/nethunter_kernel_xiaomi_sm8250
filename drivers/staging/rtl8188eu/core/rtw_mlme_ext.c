		...
		if (((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) && (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS)) {
			...
			if (psta != NULL) {
				...
				*/
				...
				if (rate_num == 0) {
					pr_info(FUNC_ADPT_FMT" RX beacon with no supported rate\n", FUNC_ADPT_ARG(padapter));
					goto _END_ONBEACON_;
				}

				psta = rtw_alloc_stainfo(pstapriv, get_addr2_ptr(pframe));
				if (psta == NULL) {
					pr_info(FUNC_ADPT_FMT" Exceed the upper limit of supported clients\n", FUNC_ADPT_ARG(padapter));
					goto _END_ONBEACON_;
				}
...
void report_add_sta_event(_adapter *padapter, unsigned char *MacAddr)
{
	...
	pr_info("report_add_sta_event: add STA\n");
...
void linked_status_chk(_adapter *padapter, u8 from_timer)
{
	...
	rtw_rson_scan_wk_cmd(padapter, RSON_SCAN_PROCESS);
		...
		if (rtw_chk_roam_flags(padapter, RTW_ROAM_ACTIVE)) {
			pr_info("signal_strength_data.avg_val = %d\n", precvpriv->signal_strength_data.avg_val);
			if (precvpriv->signal_strength_data.avg_val < pmlmepriv->roam_rssi_threshold) {
#ifdef CONFIG_RTW_80211K
...
bypass_active_keep_alive:
			...
			if (rx_chk == _FAIL) {
				...
				if (pmlmeext->retry > rx_chk_limit) {
					pr_info(FUNC_ADPT_FMT" disconnect or roaming\n",
						  FUNC_ADPT_ARG(padapter));
		...
		while (rtw_end_of_queue_search(&dlist, plist) == _FALSE) {
			...
			plist = get_next(plist);
			rtw_list_delete(&psta->list);
			pr_info(FUNC_ADPT_FMT" ibss expire "MAC_FMT"\n"
