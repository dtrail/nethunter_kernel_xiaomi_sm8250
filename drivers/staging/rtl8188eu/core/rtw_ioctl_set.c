		...
		if (select_ret == _SUCCESS) {
			...
			if (check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE) {
				...
				if (rtw_create_ibss_cmd(padapter, 0) != _SUCCESS) {
					pr_info("***Error=>do_goin: rtw_createbss_cmd status FAIL*** \n ");
					ret =  _FALSE;
					goto exit;
				}

				pmlmepriv->to_join = _FALSE;
				pr_info("***Error=> rtw_select_and_join_from_scanned_queue FAIL under STA_Mode*** \n ");
...
u8 rtw_set_802_11_bssid(_adapter *padapter, u8 *bssid)
{
	...
	pr_info("Set BSSID under fw_state=0x%08x\n", get_fwstate(pmlmepriv));
...
u8 rtw_set_802_11_ssid(_adapter *padapter, NDIS_802_11_SSID *ssid)
{
	...
	pr_info("Set SSID under fw_state=0x%08x\n", get_fwstate(pmlmepriv));
	if (check_fwstate(pmlmepriv, _FW_UNDER_SURVEY) == _TRUE)
		goto handle_tkip_countermeasure;
	else if (check_fwstate(pmlmepriv, _FW_UNDER_LINKING) == _TRUE)
		goto release_mlme_lock;
