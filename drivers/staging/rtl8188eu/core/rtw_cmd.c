...
u8 rtw_joinbss_cmd(_adapter  *padapter, struct wlan_network *pnetwork)
{
	...
	if (pmlmepriv->assoc_ssid.SsidLength == 0){
		pr_info("+Join cmd: Any SSid\n");
	} else {
		pr_info("+Join cmd: SSid=[%s]\n", pmlmepriv->assoc_ssid.Ssid);
	}

	pcmd = (struct cmd_obj*)rtw_zmalloc(sizeof(struct cmd_obj));
	if(pcmd==NULL){
		res=_FAIL;
		pr_info("rtw_joinbss_cmd: memory allocate for cmd_obj fail!!!\n");
		goto exit;
	}
	...
	if(psecnetwork==NULL)
	{
		...
		pr_info("rtw_joinbss_cmd :psecnetwork==NULL!!!\n");
...
void rtw_survey_cmd_callback(_adapter	*padapter ,  struct cmd_obj *pcmd)
{
	...
	if (pcmd->res == H2C_DROPPED) {
		...
		mlme_set_scan_to_timer(pmlmepriv, 1);
				...
				pr_info("********Error:rtw_select_and_join_from_scanned_queue Wait Sema  Fail ************\n");
		mlme_set_scan_to_timer(pmlmepriv, 1);
	}
