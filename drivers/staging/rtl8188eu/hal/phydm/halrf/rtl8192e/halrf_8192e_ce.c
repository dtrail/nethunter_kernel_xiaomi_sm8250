...
)
{
	...
	if ((iqk_result_x != 0) && (*(p_dm->band_type) == ODM_BAND_2_4G)) {
...
)
{
	...
	if (*(p_dm->mp_mode) == true) {
		...
		u16	rate	 = *(p_dm->forced_data_rate);
	...
	if (method == TXAGC) {
...
#endif
		{
			...
			/* phy_set_tx_power_level8192e(p_dm->adapter, *p_dm->channel); */
			...
			if (rf_path == RF_PATH_A) {
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_A, *p_dm->channel, CCK);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_A, *p_dm->channel, OFDM);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_A, *p_dm->channel, HT_MCS0_MCS7);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_A, *p_dm->channel, HT_MCS8_MCS15);
			} else {
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_B, *p_dm->channel, CCK);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_B, *p_dm->channel, OFDM);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_B, *p_dm->channel, HT_MCS0_MCS7);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_B, *p_dm->channel, HT_MCS8_MCS15);
			}
		}

#endif
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
		/* phy_rf6052_set_cck_tx_power(p_dm->priv, *(p_dm->channel)); */
		/* phy_rf6052_set_ofdm_tx_power(p_dm->priv, *(p_dm->channel)); */
#endif
...
)
{
	...
	indexforchannel = odm_get_right_chnl_place_for_iqk(*p_dm->channel);
...
)
{
	...
	struct dm_struct		*p_dm = &p_hal_data->odmpriv;
#elif (DM_ODM_SUPPORT_TYPE == ODM_WIN)
	struct dm_struct		*p_dm = &p_hal_data->DM_OutSrc;
#endif
#endif
