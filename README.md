...
shell:
/_ replace X with number from above _/
...
- **Kernel Hook:** Intercepts specific wakelocks at the source (`/sys/kernel/chimera_doom`). By default, the kernel blocks _nothing_ until told otherwise by the controller.
...
_Note: The kernel uses smart substring matching. If you add `telemetry`, it will automatically block `qcom_telemetry_ws`. \*\*Do not use wildcards (`_`).\*\*\*

**Userspace vs. Kernel Wakelocks (Important!):**
Modern Android (10+) aggregates App/Java wakelocks (like `gms_scheduler` from Google Play Services, Facebook, WhatsApp) before they reach the kernel. This means the C-Kernel module **cannot see or block them** directly via the blocklist text file. The kernel only sees raw hardware/driver wakelocks (e.g., `wlan`, `DIAG_WS`, `NETLINK`).

To effectively build your blocklist and save battery:

1. Let the module run for a while.
2. Check the **Live Analytics** in the WebUI.
3. Only block the wakelocks that actually appear in those stats with high "Allowed" counts! These are the true low-level kernel wakelocks triggered by your phone's unique hardware and userspace apps.
4. Java app wakelocks (like Google Play Services) are automatically and safely throttled by the `chimera_controller.sh` daemon via Android's deep standby-buckets, not by listing them here.
   Don't touch that script! The blocking of GMS locks is handled in a safe way, without any risk of breaking your phone's functionality. If you want to block userspace/Java wakelocks in Android, use a systemless module or app like NoWakeLock (\*see "Recommended Apps below)
...
# Safe to block. Stops Qualcomm/System data collection. (Usually just useless QComm telemetry. Keep an eye on phone calls functionalit, if blocked!
# DIAG_WS
telemetry
# mdm_stats
# logd
...
# --- JAVA/USERSPACE WAKELOCKS (Examples - Usually Ignored) ---
# Modern Android aggregates Android/Java wakelocks (like GMS).
# The C-Kernel module cannot see these string names directly.
# They are stopped by chimera_controller.sh (standby-buckets).
# #gms_scheduler
...
- **NoWakeLock:** https://f-droid.org/en/packages/com.js.nowakelock/
  A great addition to monitor and block userspace wakelocks, alarms and services!

- **EnforceDoze** https://f-droid.org/packages/com.akylas.enforcedoze/
  Complements the kernel very well as an additional battery saving tool!

- **Franco Kernel Manager:** https://play.google.com/store/apps/details?id=com.franco.kernel
  Pretty much the best universal kernel manager!

- **MMRL:** https://github.com/Meow-Meow-Root-Lab/MMRL
  The best tool to manage your Magisk modules! Comes with several module repos.
